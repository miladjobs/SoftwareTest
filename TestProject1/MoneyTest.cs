namespace TestProject1
{
	...
	public class MoneyTest
	{
		...
		[Test]
		public void BagMultiply2() //with fluent assertion
		{
			// {[12 CHF][7 USD]} *2 == {[24 CHF][14 USD]}
			Money[] bag = { new Money(24, "CHF"), new Money(14, "USD") };
			MoneyBag expected = new MoneyBag(bag);
			fMB1.Multiply(2).Should().Be(expected);
			fMB1.Should().Be(fMB1.Multiply(1));
			fMB1.Multiply(0).IsZero.Should().BeTrue();
		}
		...
		[Test]
		public void BagNegate2()//with fluent assertion
		{
			// {[12 CHF][7 USD]} negate == {[-12 CHF][-7 USD]}
			Money[] bag = { new Money(-12, "CHF"), new Money(-7, "USD") };
			MoneyBag expected = new MoneyBag(bag);
			fMB1.Negate().Should().Be(expected);
		}
		...
		public void MoneyBagEquals()
		{
			...
			Assert.IsTrue(!f14CHF.Equals(fMB1));
			Assert.IsTrue(!fMB1.Equals(fMB2));
		}

		[Test]
		public void MoneyBagEquals2()//with fluent assertion
		{
			//NOTE: Normally we use Assert.AreEqual to test whether two
			// objects are equal. But here we are testing the MoneyBag.Equals()
			// method itself, so using AreEqual would not serve the purpose.
			...
			MoneyBag equal = new MoneyBag(new Money(12, "CHF"), new Money(7, "USD"));
			fMB1.Should().NotBeNull().And.Subject.Should().BeEquivalentTo<MoneyBag>(equal);
			fMB1.Should().Subject.Should().NotBeEquivalentTo(f12CHF);
			fMB1.Should().Subject.Should().NotBeEquivalentTo(f14CHF);
		...
		public void Print()
		{
			Assert.IsNotNull(f12CHF.ToString());
			Assert.AreNotEqual(f12CHF.ToString(), "");
			Assert.AreEqual(f12CHF.ToString(), "[12 CHF]");
		}

		[Test]
		public void Print2() //with fluent assertion
		{
			f12CHF.ToString().Should().NotBeNullOrWhiteSpace().And.Subject.Should().Be("[12 CHF]");
		}
